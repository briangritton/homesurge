...
function PersonalInfoForm(props) {
    ...
    try {
      ...
      if (window.dataLayer) {
        window.dataLayer.push({
          ...
          variantIndex,
          variantName: `PersonalInfoForm${variantIndex + 1}`
        });
      }
    } catch (error) {
  ...
  if (selectedVariant === null) return <PersonalInfoForm1 {...props} />;

  // Try to render selected variant with fallback
  try {
    // ========== TOGGLE SECTION ==========
    // OPTION 1: Use all variants
    // const variants = [PersonalInfoForm1, PersonalInfoForm2];
    
    // OPTION 2: Use only specific variants (comment out option 1 and uncomment below)
    const variants = [PersonalInfoForm1]; // Only use variant 1
    // const variants = [PersonalInfoForm2]; // Only use variant 2
    // const variants = [PersonalInfoForm1, PersonalInfoForm1]; // Looks like A/B testing but actually always uses variant 1
    // ====================================
    ...
    return <PersonalInfoForm1 {...props} />;
  }
}
